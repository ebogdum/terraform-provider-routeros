...
type LogDataSource struct{ reg *client.Registry }

type LogDSModel struct {
	Router  types.String `tfsdk:"router"`
	Filter  types.Map    `tfsdk:"filter"`
	Records types.List   `tfsdk:"records"`
}

func NewLogDataSource() datasource.DataSource { return &LogDataSource{} }
...
func (d *LogDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Lists records from /log.",
...
func (d *LogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	rows, err := c.List(ctx, "/log", opts...)
	if err != nil {
		resp.Diagnostics.AddError("Read /log failed", err.Error())
		return
	}
