...
# Resource: routeros_interface_bridge_nat

Manages the RouterOS `/interface/bridge/nat` menu.

## Example Usage

```terraform
resource "routeros_interface_bridge_nat" "nat_example" {
...
* `to_arp_reply_mac_address` - (Optional) Type: `string`. RouterOS `to-arp-reply-mac-address`.
* `to_dst_mac_address` - (Optional) Type: `string`. RouterOS `to-dst-mac-address`.
* `to_src_mac_address` - (Optional) Type: `string`. RouterOS `to-src-mac-address`.
...
```sh
# Default router, .id = *3
terraform import routeros_interface_bridge_nat.example '*3'

# Named router
terraform import routeros_interface_bridge_nat.example 'home/*3'

# By natural key (the resource's `name` attribute, when present)
terraform import routeros_interface_bridge_nat.example 'home/my-resource-name'
```
