...
* `accounting_port` - (Optional) Type: `int`. RADIUS server port used for accounting
...
* `authentication_port` - (Optional) Type: `int`. RADIUS server port used for authentication.
...
* `protocol` - (Optional) Type: `string`. Specifies the protocol to use when communicating with the RADIUS Server.
* `radsec` - (Read-only) Type: `string`.
