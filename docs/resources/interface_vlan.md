...
* `loop_protect` - (Optional) Type: `string`. Loop protection: `default`, `on` or `off`.
* `loop_protect_disable_time` - (Optional) Type: `string`. How long the interface stays disabled after a loop is detected. `0s` disables the timeout.
* `loop_protect_send_interval` - (Optional) Type: `string`. Interval between loop-protect probe packets.
...
* `vlan_id` - (Required) Type: `string`. Virtual LAN identifier or tag that is used to distinguish VLANs. Must be equal for all computers that belong to the same VLAN.
