...
# Resource: routeros_ipv6_firewall_nat

Manages the RouterOS `/ipv6/firewall/nat` menu.

## Example Usage

```terraform
resource "routeros_ipv6_firewall_nat" "nat_example" {
...
* `connection_state` - (Optional) Type: `string`. RouterOS `connection-state`.
...
* `to_address` - (Optional) Type: `string`. RouterOS `to-address`.
...
```sh
# Default router, .id = *3
terraform import routeros_ipv6_firewall_nat.example '*3'

# Named router
terraform import routeros_ipv6_firewall_nat.example 'home/*3'

# By natural key (the resource's `name` attribute, when present)
terraform import routeros_ipv6_firewall_nat.example 'home/my-resource-name'
```
