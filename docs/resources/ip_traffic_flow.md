...
* `active_flow_timeout` - (Optional) Type: `string`. Maximum life-time of a flow.
...
* `inactive_flow_timeout` - (Optional) Type: `string`. How long to keep the flow active, if it is idle. If a connection does not see any packet within this timeout, then traffic-flow will send a packet out as a new flow. If this timeout is too small it can create a significant amount of flows and overflow the buffer.
